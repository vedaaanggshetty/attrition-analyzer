...
/**
 * Validates JWTs issued by Authentication Service. The Gateway never issues
 * its own tokens - only Authentication does - so this class is
 * parse/verify-only, using the same shared signing secret ({@code
 * JWT_SECRET}) every service is configured with. Mirrors
 * user-profile-service's class of the same name - each service owns its own
 * copy since there is no shared library in this project.
 */
@Component
